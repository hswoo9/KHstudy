...
window.onload = () => {
    ...
    btn5.addEventListener('click', () => {
        ...
        for (const i in students) {
            console.log(i);
            div3.innerHTML += 
                `이름: ${students[i].name}, 총점: ${students[i].getSum()}, 평균: ${students[i].getAvg()}<br>`;
        }
    });
    ...
    btn7.addEventListener('click', () => {
        ...
        div5.innerHTML += `date3 : ${date3}<br><br>`;
        ...
        div5.innerHTML += `now.toLocaleString('en-US') : ${now.toLocaleString('zh-hk')}<br>`;
    });
};